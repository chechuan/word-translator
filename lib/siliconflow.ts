...
async function complete(model: string, prompt: string, options: { json?: boolean; timeoutMs?: number; maxTokens?: number } = {}) {
  ...
  if (!key) throw new Error("The translation API key is not configured.");
  ...
  if (!response.ok) throw new Error(response.status === 429 ? "The translation service is rate limited. Please try again." : "The translation service is temporarily unavailable.");
  const body = await response.json() as { choices?: Array<{ finish_reason?: string; message?: { content?: string } }> };
  const choice = body.choices?.[0];
  if (choice?.finish_reason !== "stop" || !choice.message?.content?.trim()) throw new Error("The translation result was incomplete. Please try again.");
  return choice.message.content.trim();
}

export async function findPhrases(text: string) {
  const model = process.env.ANALYSIS_MODEL;
  if (!model) throw new Error("The analysis model is not configured.");
...
export async function translateBatch(sources: string[]) {
  ...
  if (!model) throw new Error("The translation model is not configured.");
  try {
